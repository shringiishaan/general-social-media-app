...
import { IUserCareer, IIndustry, IJobRole } from "../../../services/interfaces"
...
type EditMyCareerScreenProps = {
    ...
    selectedCareer: IUserCareer | undefined,
    handleBackPress: () => void
}

export const EditCareerScreen: React.FC<EditMyCareerScreenProps> = ({ navigation, selectedCareer, handleBackPress }) => {
    ...
    React.useEffect(() => {
        loadMyIndustry()
        loadMyJobRole()
        setCareerYear(selectedCareer && (selectedCareer.year !== null ? selectedCareer?.year?.toString() : ""))
    }, [])
    ...
    const loadMyIndustry = (): Promise<void> => {
        return new Promise<void>((resolve, reject) => {
            ...
            APIService.getAllIndustries().then((allIndustries) => {
                ...
                const selectedIndustryFromAll = allIndustries.find(industry => industry.id === selectedCareer?.industryId)
                setSelectedIndustry(selectedIndustryFromAll)
                resolve()
            }).catch((error) => {
                setError(error)
                reject(error)
    ...
    const loadMyJobRole = (): Promise<void> => {
        return new Promise<void>((resolve, reject) => {
            ...
            APIService.getAllJobRoles().then((allJobRoles) => {
                ...
                const selectedJobRoleFromAll = allJobRoles.find(jobRole => jobRole.id === selectedCareer?.jobRoleId)
                setselectedJobRoles(selectedJobRoleFromAll)
                resolve()
            }).catch((error) => {
                setError(error)
                reject(error)
    ...
    const handleUpdateCareer = (): Promise<void> => {
        return new Promise<void>((resolve, reject) => {
            setLoading(true)
            setError(null)

            const updatedCareers: IUserCareer[] = (myCareer ?? []).map(career => {
                if (selectedCareer && career.id === selectedCareer.id) {
                    return {
                        ...career,
                        industryId: selectedIndustry?.id ?? career.industryId,
                        jobRoleId: selectedJobRoles?.id ?? career.jobRoleId,
                        priority: 0,
                        year: careerYear ? parseInt(careerYear, 10) : career.year,
                    }
                }
                return career
            })

            APIService.updateMyCareers(updatedCareers).then((myCareer) => {
                setUserCareer(updatedCareers)
                resolve()
            }).catch((error) => {
                setError(error)
                reject(error)
    ...
    const handleDeleteCareer = (careerId: number): Promise<void> => {
        return new Promise<void>((resolve, reject) => {
            setLoading(true)
            setError(null)

            if (myCareer && myCareer.length > 0) {
                APIService.deleteMyCareerById(careerId)
                    .then((success) => {
                        if (success) {
                            resolve()
                            handleBackPress()
                ...
                <View style={ {
                    ...
                    <View>
                        <Icon name="delete"
                            ...
                            size={ 30 }
                            onPress={ () =>
                                handleDeleteCareer(selectedCareer?.id ?? 0)
                            } />
                    </View>
                </View>
            </View >
